...
pkgver=1
pkgrel=4
pkgdesc="(! use only with lightdm !) Setup nvidia and intel for optimus based laptops without bumblebee (current nvidia driver) (! use only with lightdm !)"
license=("none")
kernelvers="$(uname -r | awk -F "." '{print $1 $2}')"
depends=('xf86-video-intel' 'python' 'lightdm' "linux${kernelvers}-nvidia" 'lib32-nvidia-utils' 'nvidia-utils')
#optdepends=()
makedepends=('python')
